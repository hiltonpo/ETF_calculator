...
const {
  monthlyInvest, years, rate0050, rate2x,
  setMonthlyInvest, setYears, setRate0050, setRate2x,
} = useInvestmentCalc()
...
<template>
  <UCard class="animate-fade-up-1 mb-6" :ui="{ base: 'bg-gray-900 border border-white/8', body: { padding: 'p-6' } }">
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
      <div v-for="(slider, i) in sliders" :key="i">
        <div class="flex justify-between items-center mb-2">
          <span class="font-mono text-[11px] tracking-widest uppercase text-gray-500">
            {{ slider.label }}
          </span>
        ...
        <input
          ...
          :style="`--pct: ${((slider.value - slider.min) / (slider.max - slider.min)) * 100}%`"
          @input="slider.onUpdate(Number(($event.target as HTMLInputElement).value))"
        />
      </div>
    </div>
  </UCard>
